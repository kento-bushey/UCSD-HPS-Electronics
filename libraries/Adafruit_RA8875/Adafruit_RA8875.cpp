...
void Adafruit_RA8875::drawBezier(Point start, Point end, Point control, uint16_t resolution, uint16_t color){
  struct Point prev = start;
  struct Point cur = start;
  for(uint16_t i = 1; i<=resolution; i++){
    float t = max(0.0f,min(1.0f,((float)i)/((float)resolution)));
    struct Point PointTemp1 = { .x=lerp(start.x, control.x, t), .y=lerp(start.y, control.y, t)};
    struct Point PointTemp2 = { .x=lerp(control.x, end.x, t), .y=lerp(control.y, end.y, t) };
    cur.x = lerp(PointTemp1.x, PointTemp2.x,t);
    cur.y = lerp(PointTemp1.y, PointTemp2.y,t);
